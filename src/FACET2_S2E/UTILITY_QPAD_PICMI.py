...
def to_scientific_notation(value, nprec = 4):
...
class Neutral(picmistandard.PICMI_Species):
	...
	def fill_dict(self, keyvals, if_lasers):
		...
		if(not isinstance(self.initial_distribution, OpenPMDFileDistribution)):
...
class GaussianBunchDistribution(picmistandard.PICMI_GaussianBunchDistribution):
	...
	def fill_dict(self,keyvals):
		...
		for j in range(3):
			keyvals['range' + str(j+1)] = [to_scientific_notation(-4 * self.rms_bunch_size[j] +self.centroid_position[j]),\
...
class FieldDiagnostic(picmistandard.PICMI_FieldDiagnostic):
	...
	def init(self,kw):
		...
		if('rho' in self.data_list):
			self.source_list += ['charge_cyl_m', 'ion_cyl_m']
